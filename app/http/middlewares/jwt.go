...
func JWTWithConfig(config JWTConfig) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			user := &serializers.LoggedInUser{}
			ctx := context.Background()

			// Check if access_uuid corresponds to user_id in Redis
			if !methods.IsEmpty(conf.Redis().AccessUuidPrefix + tokenDetails.AccessUuid) {
				redisUser, _ = cache.Client().Get(ctx, conf.Redis().AccessUuidPrefix+tokenDetails.AccessUuid)
